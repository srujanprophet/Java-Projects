...
class medAI extends Player {
    ...
    protected String makeMove(String board) {
        ...
        int winIdx = moveLogic(board, player);
...
class hardAI extends Player {
    public hardAI(char ch) {
        super(ch);
    }

    private int evaluate(String board) {
        if (Main.isWinner(board, player)) {
            return 10;
        } else if (Main.isWinner(board, opponent)) {
            return -10;
        } else {
            return 0;
        }
    ...
    private int minimax(char[] arr, int depth, boolean isMax) {
        int score = evaluate(String.valueOf(arr));

        if (score == 10) return score;
        if (score == -10) return score;
        if (!String.valueOf(arr).contains("_")) return 0;
...
class playerFactory {
    public Player getPlayer(String type, char turn) {
        return switch (type) {
            case "easy" -> new ezAI(turn);
            case "medium" -> new medAI(turn);
            case "hard" -> new hardAI(turn);
            default -> new Human(turn);
        };
    }
}
