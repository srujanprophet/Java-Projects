...
public class Main {
    ...
    static char[][] fogBoard = new char[10][10];
    static HashMap<String, ArrayList<Location>> shipLocations = new HashMap<>();
    ;
    ...
    public static void place(int grid, int start, int l, String type, char alignment) {
    ...
    public static void placeShip(String type, int l) {
        while (true) {
            ...
            if (canPlace(loc1.x, loc1.y, loc2.x, loc2.y, l, type)) {
                if (loc1.x == loc2.x) {
                    place(loc1.x, Math.min(loc1.y, loc2.y), l, type,'H');
                    break;
                } else if (loc1.y == loc2.y) {
                    place(loc1.y, Math.min(loc1.x, loc2.x), l, type, 'V');
                    break;
                }
            }
        }
    ...
    public static void placeShips(char[][] board) {
        for (Ship ship: Ship.values()) {
            placeShip(ship.getShipType(), ship.getShipLen());
            printBoard(board);
        }
    }

    public static boolean hasShipSunk() {
